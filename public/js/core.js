// Auto-split from app.js — module: core. Behavior-preserving relocation only.
import { marked } from "/vendor/marked.esm.js";
import DOMPurify from "/vendor/purify.es.mjs";
// core is a LEAF primitives module: it must NOT import feature modules (auth/chat/
// loaders/shell), or it forms import cycles. Session-expiry teardown is orchestration,
// so it lives in lifecycle.js and registers itself here via setSessionExpiredHandler;
// the api()/SSE 401 paths call triggerSessionExpired().
...
export function setAbort(value) {
...
/* ============================================================ Theme */
// Light/dark is resolved HERE (core runs earliest, before render) and written to
// <html data-theme>. Preference is per-device in localStorage: "system" (default,
// follows the OS) | "light" | "dark". The dark token block lives in
// styles/00-tokens.css under :root[data-theme="dark"] (single source).
...
applyTheme();
// In "system" mode, track live OS changes; an explicit light/dark pick ignores them.
...
// Registration hook for session-expiry teardown. The orchestration itself
// (stop streams, reset state, render auth) lives in lifecycle.js — keeping core a
// leaf. triggerSessionExpired() is a no-op until lifecycle registers the handler;
// in practice lifecycle is evaluated by the app.js entry before any api() call runs.
...
let notifyWrap = null;
export function notify(message, kind = "error", opts = {}) {
...
export function setFormBusy(root, busy) {
  if (!root) return;
...
export function wireExpander(btn, contents, load, { once = false } = {}) {
  let loaded = false;
  const runLoad = async () => {
    btn.disabled = true;
    try {
