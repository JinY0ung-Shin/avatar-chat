  ...
  if (request.browserEnabled) {
    lines.push(
      "Browser control: you can drive THIS user's own browser with `mcp__browser__snapshot` / `read_text`" +
        ...
        ` To put LONG or rich TEXT into an editor (a Monaco/CodeMirror code editor, a contentEditable body — roughly anything over 1KB), use \`mcp__browser__copy_text\` and the SAME staged-page flow: new_tab the URL it returns, click '클립보드로 복사', VERIFY with \`list_tabs\` that the title says "COPIED", select-all first if you are REPLACING the editor's content, then paste. A long \`type\` into such an editor can be silently truncated, which is why paste is the reliable route — but it OVERWRITES the user's own clipboard, so only do it when the task needs it.` +
