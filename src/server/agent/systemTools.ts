    ...
    tool(
      ...
      async () => {
        ...
        const lines = [
          ...
          `- Confluence host: ${ctx.config.confluenceUrl ? `set (mcp__confluence__* is READ-ONLY — no page creation, editing, or deletion exists; ${ctx.browserEnabled ? "to write, drive Confluence in the user's own browser with mcp__browser__* — including mcp__browser__copy_image then press_key Ctrl+V to paste an image into a page body" : "writing would need browser control, which is unavailable in this run"})` : "(none)"}`,
