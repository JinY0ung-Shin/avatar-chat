...
- `click_at` — 접근성 트리에 잡히지 않는 대상(canvas 에디터, 지도, 그려진 차트)을 위한
  탈출구입니다. 직전 **뷰포트** 스크린샷 이미지 위에서 잰 픽셀 좌표를 받아, 캡처 때 적용된
...
- 클릭 직전 `DOM.getNodeForLocation` + `DOM.describeNode`(둘 다 읽기 전용, allowlist에
  추가된 유일한 메서드)로 그 지점의 요소를 읽어 "무엇에 맞았는지"를 결과에 함께
  보고합니다 — 눈감고 클릭한 뒤 성공을 가정하지 않는다는 기존 원칙의 좌표 클릭
  버전입니다. 요소를 식별하지 못하면 침묵하는 대신 결과에 "식별 실패" 경고를 명시해
  성공처럼 읽히지 않게 합니다. 임의 JS 실행 없음(`Runtime.*` 부재)은 그대로입니다.
- 대화의 모델이 이미지를 받을 수 있을 때만 서버가 도구를 엽니다(스크린샷과 같은 게이트 —
  좌표의 출처가 스크린샷이기 때문). 감사 로그에는 클릭 좌표가 남습니다.
