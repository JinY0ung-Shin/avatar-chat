...
- **JS 실행 경로가 없습니다.** `CDP_ALLOWLIST`는 기본 거부이고 `Runtime.*`·`Storage.*`가 없으며,
  `Network.*` 중에서는 오직 동의 기반·현재 오리진 한정의 `Network.getCookies`(read_cookies 도구)만,
  그리고 `DOMStorage` 도메인에서는 `DOMStorage.getDOMStorageItems`(read_storage 도구, `DOMStorage.enable`은
  제외)만 허용됩니다. 이 쿠키·저장소 읽기는 사이트별로, 그리고 종류별(쿠키 / localStorage / sessionStorage)로
