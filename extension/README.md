...
- 스크립트가 값을 관리하는 검색창(map.naver.com 등)이 `commands: ["selectAll"]` 전체 선택을
  ...
  다만 이제는 조용히 덧붙이지 않고 값 검증이 잡아서 폴백을 거치거나 정직하게 실패합니다
...
`clear`는 **검증되는 쓰기**입니다(0.13.0). 스크립트가 값을 직접 관리하는 입력창
(React 검색창 등)에서는 전체 선택이 조용히 무시돼 교체가 아니라 덧붙이기가 되곤 했습니다
("광교" + "카페거리" → "광교카페거리"). 그래서 전체 선택은 단축키와 함께 CDP의
편집 명령(`commands: ["selectAll"]`)까지 보내고, 쓰기 후 필드 값을 **다시 읽어** 확인합니다
(`Accessibility.getPartialAXTree` — 노드 하나만 읽는 읽기 전용 메서드로 allowlist에 추가).
옛 값이 앞이나 뒤에 남아 있으면 사람이 하듯 End → Backspace로 지우고 다시 입력하며,
그래도 남으면 조용히 성공했다고 하지 않고 **지금 필드에 무엇이 들어 있는지 밝히며
실패**합니다(필드 자체의 X 지우기 버튼을 쓰거나 사용자에게 넘기라고 안내). clear를 주지
않은 기본 삽입 경로는 그대로입니다 — 읽기도 지연도 추가되지 않습니다.
...
- `fill_form` — 여러 필드를 **한 호출**로 순서대로 채우고 스냅샷은 마지막에 한 번만
  ...
  전체 선택(Ctrl+A/⌘A) 후 덮어쓰고, `type`과 같은 검증·폴백을 거칩니다(위 "텍스트 입력 경로").
  제출은 하지 않습니다 — 제출 버튼 클릭은 별도입니다.
