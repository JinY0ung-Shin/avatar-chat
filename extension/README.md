...
- **JS 실행 경로가 없습니다.** `CDP_ALLOWLIST`는 기본 거부이고 `Runtime.*`·`Storage.*`가 없으며,
  ...
  허용됩니다. 이 쿠키 읽기는 사이트별로 처음 읽을 때 브라우저에 확인 팝업이 떠서 허용해야 실행되며, 한 번
  허용하면 이번 브라우저 세션 동안 같은 사이트는 다시 묻지 않습니다(확장 설정에서 취소하거나 브라우저를
  닫으면 사라집니다). 감사 로그에는 쿠키 값이 아니라 이름·개수만 남습니다. 요소는 접근성 트리의
  `backendNodeId`로만
  지목합니다.
